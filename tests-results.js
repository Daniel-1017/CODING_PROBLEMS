...
function testResult(array) {
  ...
  const average = +(
    array.reduce((acc, mark) => {
      if (mark > 8) marks.h++;
      else if (mark > 6 && mark < 9) marks.a++;
      else if (mark < 7) marks.l++;
      return acc + mark;
    }, 0) / array.length
  ).toFixed(3);
  console.log(average, marks);
  if (marks.h === array.length) return [average, marks, "They did well"];
  else return [average, marks];
}
