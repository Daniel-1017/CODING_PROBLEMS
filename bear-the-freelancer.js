...
function bearDollars(arr) {
  // CLIENT RATES
  const CLOSE_FRIEND_RATE = 25   // $
  const FRIEND_RATE = 50         // $
  const ACQUAINTANCE_RATE = 100  // $
  const OTHER_CLIENTS_RATE = 125 // $

  return arr.map(client => {
    const [hours, howClose] = client
    
    switch (howClose.toLowerCase()) {
      case "close friend":
        return hours * CLOSE_FRIEND_RATE
      case "friend":
        return hours * FRIEND_RATE
      case "acquaintance":
        return hours * ACQUAINTANCE_RATE
      default:
        return hours * OTHER_CLIENTS_RATE
    }
