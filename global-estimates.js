...
const globalEstimate = estimates => {
    ...
    const getSum = nums => nums.reduce((acc, n) => acc + n, 0);
    const getAvg = nums => getSum(nums) / nums.length;

    const mins = estimates.map(est => Math.min(...est));
    const avgs = estimates.map(est => getAvg(est));
    const maxs = estimates.map(est => Math.max(...est));

    return [mins, avgs, maxs].map(getSum);
}