...
function center (string, width, fill = ' ') {
  ...
  for (let i = 0; i < Math.ceil((width - string.length) / 2); i++) firstPart += fill

  for (let i = 0; i < Math.floor((width - string.length) / 2); i++) secondPart += fill
...
// SECOND SOLUTION
function center (string, width, fill = ' ') {
  return fill.repeat(Math.ceil((width - string.length) / 2)) + string + fill.repeat(Math.floor((width - string.length) / 2))
}