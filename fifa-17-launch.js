...
const fifa = (ticket, results) => {
  const ticketWinners = Object.keys(ticket);
  let won = 0,
    winner = "";

  for (let i = 0; i < results.length; i++) {
    const [home, away] = results[i].split("-");

    if (home - away > 0) winner = "Home";
    else if (home - away < 0) winner = "Away";
    else winner = "Draw";

    if (winner === ticketWinners[i]) won += +ticket[winner].substr(1);
  }
