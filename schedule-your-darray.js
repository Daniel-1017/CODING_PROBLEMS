...
const dayPlan = (hours, tasks, duration) => {
  ...
  const minutesToFinishTasks = tasks * duration;
  const freeTime = (workingMinutes - minutesToFinishTasks) / (tasks - 1);

  for (let i = 0; i < tasks; i++) {
    res.push(duration, Math.round(freeTime));
  }

  if (!res.length) return [];
  if (freeTime < 0) return "You're not sleeping tonight!";
