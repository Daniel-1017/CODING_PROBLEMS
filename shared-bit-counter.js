...
function sharedBits(a, b) {
  ...
  aBits = aBits.padStart(Math.max(aBits.length, bBits.length, 0));
  bBits = bBits.padStart(Math.max(aBits.length, bBits.length, 0));
  let sharedBits = 0;
  for (let i = 0; i < aBits.length; i++) {
    if (aBits[i] == 1 && bBits[i] == 1) sharedBits++;
    if (sharedBits > 1) return true;
  }
  return false;
}
