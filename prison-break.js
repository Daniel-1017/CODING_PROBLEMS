...
const invert = arr => arr.map(el => !el);

const freedPrisoners = prison => {
    if (!prison[0]) return 0;
    
    let freedPrisoners = 1;
    prison = invert(prison);

    for (let i = 1; i < prison.length; i++) if (prison[i]) (freedPrisoners++, prison = invert(prison));

    return freedPrisoners;
}

console.log(freedPrisoners([true, true, false, false, false, true, false]));