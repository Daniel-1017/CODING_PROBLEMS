...
const window = (length, offset, list) => !length ? Array(Math.ceil((list.length + 1) / offset)).fill([]) : list.length >= length ? [list.slice(0, length), ...window(length, offset, list.slice(offset))] : [];
