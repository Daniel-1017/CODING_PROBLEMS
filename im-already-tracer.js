...
const teamComp = heroes => {
    if (heroes.length !== 6) throw new Error("GG");

    const heroCount = { TANK: 0, DAMAGE: 0, SUPPORT: 0 }, heroTeam = [];

    const isValidHero = hero => [...TANK, ...DAMAGE, ...SUPPORT].includes(hero);
    const getHeroCategory = hero => {
        if (TANK.includes(hero)) {
            return "TANK";
        } else if (DAMAGE.includes(hero)) {
            return "DAMAGE";
        } else if (SUPPORT.includes(hero)) {
            return "SUPPORT";
        }
    }
    const heroInTeamAlready = hero => heroTeam.includes(hero);

    for (const hero of heroes) {
        if (!isValidHero(hero) || heroInTeamAlready(hero)) throw new Error("GG");

        heroTeam.push(hero);

        const heroCategory = getHeroCategory(hero);
        heroCount[heroCategory]++;
    }

    return Object.values(heroCount);
}