...
const secondLargest = array => {
  ...
  array = array.filter(item => /[0-9]/.test(parseInt(item))).map(Number);

  const largestNumber = Math.max(...array);
  let secondLargestNumber = Math.min(...array);

  for (let n of array) {
    if (n < largestNumber && n > secondLargestNumber) secondLargestNumber = n;
  }
